...
def get_top_3_books_to_article(abstract):
    """
    Calculate the similarity between an article's abstract and books abstracts using tf-idf computation and cosine similarity.
    Return the 3 most similar books to an article based on this computation.
    If the abstract is empty, return "None"

    Args:
        abstract (str): abstract of the article

    Returns:
        top_3_books (list of dict): list of the 3 most similar books to an article
    """
    ...
    if abstract != "":
        ...
        top_3_books = top_3_books.to_json(orient='records', lines=False, indent=0)
        top_3_books = json.loads(top_3_books)
        return top_3_books
